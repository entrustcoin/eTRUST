...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake, int64_t* pFees)
{
    ...
    if (!fProofOfStake)
    {
        ...
        CPubKey pubkey;
        //if (!reservekey.GetReservedKey(pubkey).GetID())
        if (!reservekey.GetReservedKey(pubkey))
            return NULL;
        //txNew.vout[0].scriptPubKey.SetDestination(reservekey.GetReservedKey(pubkey).GetID());
...
void StakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("entrustcoin-miner");
