...
const string strMessageMagic = "Entrustcoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (nHeight <= 0)
        ...
        {
            nSubsidy = 20008.0001 * COIN;
        }
    else if (nHeight <= LAST_FAIR_LAUNCH_BLOCK)
        {
            nSubsidy = 8.1 * COIN;
        }
    else
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if(IsProofOfWork())
    {
        ...
        if (vtx[0].vout[1].scriptPubKey != scriptPubKey)
            return error("ConnectBlock() : coinbase does not pay to the Entrust address)");
        if (vtx[0].vout[1].nValue < devCoin)
            return error("ConnectBlock() : coinbase does not pay enough to Entrust addresss");
    }
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProofOfWork() && nHeight > LAST_FAIR_LAUNCH_BLOCK)
...
// entrustcoin: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Entrustcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        */
        ...
        const char* pszTimestamp = "Obama is being a Nazi about Bitcoin.";
        CTransaction txNew;
        txNew.nTime = 1428158704;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1428158704;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 2623698: 2623698;
        uint256 hash = block.GetHash();
        printf("%s\n", hash.ToString().c_str());
        printf("%s\n", hashGenesisBlock.ToString().c_str());
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
        assert(block.hashMerkleRoot == uint256("0x62a4f50cf898a1b41b0a82b44fb5b9b1562d3f300513e515474a44f9021f202f"));
...
unsigned char pchMessageStart[4] = { 0xff, 0xf5, 0xff, 0xf5 };
